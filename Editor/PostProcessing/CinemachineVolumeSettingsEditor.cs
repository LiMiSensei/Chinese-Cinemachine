    ...
    class CinemachineVolumeSettingsEditor : UnityEditor.Editor
    {
        ...
        public override VisualElement CreateInspectorGUI()
        {
...
#if CINEMACHINE_HDRP
                "对焦跟踪（Focus Tracking）要求配置文件中存在已激活的景深覆盖（Depth Of Field override）设置，且需满足以下两种情况之一： "
                    + "已激活对焦距离（Focus Distance）和对焦模式（Focus Mode），并将对焦模式设置为 “使用物理相机（Use Physical Camera）”；",
#else
                "对焦跟踪（Focus Tracking）要求配置文件中存在已激活的景深覆盖（Depth Of Field override）设置，且需满足以下两种情况之一："
                    + "已激活对焦距离（Focus Distance）和模式（Mode），并将该模式设置为 “散景（Bokeh）”。",
#endif
            ...
            var missingProfileHelp = ux.AddChild(new HelpBox(
                "通过选择资源来指定现有的体积配置文件，或通过点击“新建”按钮创建一个新的配置文件。",

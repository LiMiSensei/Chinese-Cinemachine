...
namespace Unity.Cinemachine.Editor
{
    ...
    class CinemachinePostProcessingEditor : UnityEditor.Editor
    {
        ...
        public override VisualElement CreateInspectorGUI()
        {
            ...
            ux.Add(new HelpBox(
                "此组件在 HDRP（高清渲染管线）和 URP（通用渲染管线）项目中无效。请改用 CinemachineVolumeSettings 组件。",
            ...
            var trackingHelp = ux.AddChild(new HelpBox(
                "对焦跟踪（Focus Tracking）需要配置文件中存在已激活的景深（DepthOfField）/ 对焦距离（FocusDistance）效果。",
                HelpBoxMessageType.Warning));
            ...
            var newButton = row.Contents.AddChild(new Button(() =>
            {
                ...
                path = EditorUtility.SaveFilePanelInProject(
                    "创建后期处理配置文件", Path.GetFileName(path), "asset",
                    "此资源将包含后期处理设置");
                if (path.Length > 0)
                {
            ...
            var missingProfileHelp = ux.AddChild(new HelpBox(
                "通过选择资源来指定现有的后期处理配置文件，或点击“新建”按钮创建一个新的配置文件。",

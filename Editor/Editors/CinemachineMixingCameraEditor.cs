...
namespace Unity.Cinemachine.Editor
{
    ...
    class CinemachineMixingCameraEditor : CinemachineVirtualCameraBaseEditor
    {
        ...
        protected override void AddInspectorProperties(VisualElement ux)
        {
            ...
            var noChildrenHelp = ux.AddChild(new HelpBox("没有 CinemachineCamera 子对象", HelpBoxMessageType.Warning));
            var noWeightHelp = ux.AddChild(new HelpBox("没有激活的输入通道", HelpBoxMessageType.Warning));

            var container = ux.AddChild(new VisualElement());
            container.AddHeader("混合结果");

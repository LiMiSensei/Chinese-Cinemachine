    ...
    class CinemachineConfiner2DEditor : UnityEditor.Editor
    {
        ...
        public override VisualElement CreateInspectorGUI()
        {
            ...
            var boundsHelp = ux.AddChild(new HelpBox(
                "2D 边界形状必须是 PolygonCollider2D（多边形碰撞体 2D）、BoxCollider2D（盒形碰撞体 2D）或 CompositeCollider2D（复合碰撞体 2D）。",
                HelpBoxMessageType.Warning));
            var polygonsHelp = ux.AddChild(new HelpBox(
                "CompositeCollider2D（复合碰撞体 2D）的几何类型必须为多边形（Polygons）。",
                HelpBoxMessageType.Warning));
            var invalidCollider2D = ux.AddChild(new HelpBox(
                "输入的 Collider2D（碰撞体 2D）无效，它没有顶点。",
                HelpBoxMessageType.Warning));
            ...
            ux.TrackAnyUserActivity(() =>
            {
                ...
                invalidCollider2D.SetVisible(c != null && Target.BoundingShapeIsBaked && Target.IsConfinerOvenNull());
            });
            ...
            var bakeTimeout = ux.AddChild(new HelpBox(
                "多边形骨架计算超时。限制器结果可能不完整。"
                + "\n\n若要解决此问题，请减少限制形状中的顶点数量，"
                + "或设置 MaxWindowSize（最大窗口大小）参数以限制骨架计算。",
                HelpBoxMessageType.Warning));
            ...
            {
                text = "失效边界形状缓存",
                tooltip = "使 Confiner2D 缓存失效，以便下一帧重新计算缓存。\n"
                    + "当输入的边界形状发生变化时,请调用此功能。 "
                    + "（如非均匀缩放、旋转，或顶点被移动、添加、删除）"
            });

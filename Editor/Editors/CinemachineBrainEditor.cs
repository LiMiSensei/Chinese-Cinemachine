...
namespace Unity.Cinemachine.Editor
{
    ...
    class CinemachineBrainEditor : UnityEditor.Editor
    {
        ...
        public override VisualElement CreateInspectorGUI()
        {
            ...
            var brainModesWarning = ux.AddChild(new HelpBox("混合更新方式中的 <b>固定更新</b> 设置仅适用于"
                + "更新方式同样为  <b>固定更新</b> 的情况, 用于解决特定的混合问题\n"
                + "<b>延迟更新</b> 通常是推荐的设置。", HelpBoxMessageType.Warning));

            // Show the active camera and blend
            var row = ux.AddChild(new InspectorUtility.LeftRightRow());
            row.Left.Add(new Label("实时相机")
                { tooltip = "当前处于激活状态的 Cinemachine 相机", style = { alignSelf = Align.Center, flexGrow = 1 }});
            ...
            row.Left.Add(new Label("实时混合")
                { tooltip = "当前激活的混合状态（若存在）", style = { alignSelf = Align.Center, flexGrow = 1 }});

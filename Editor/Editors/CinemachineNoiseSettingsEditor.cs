...
namespace Unity.Cinemachine.Editor
{
    ...
    class NoiseSettingsEditor : UnityEditor.Editor
    {
        ...
        GUIContent mAnimatedLabel = new GUIContent("动画", "为噪点信号预览设置动画");
        ...
        public override void OnInspectorGUI()
        {
            ...
            mPreviewTime = EditorGUI.Slider(r, "预览时间", mPreviewTime, 0.01f, 10f);
            ...
            mPreviewHeight = EditorGUI.Slider(r, "预览高度", mPreviewHeight, 1f, 10f);
            r.x += r.width + hSpace; r.width = labelWidth;
            mAnimatedPreview = EditorGUI.ToggleLeft(r, mAnimatedLabel, mAnimatedPreview);
            EditorGUILayout.Separator();

            r = EditorGUILayout.GetControlRect();
            EditorGUI.LabelField(r, "位置噪声 - 振幅单位为距离单位", EditorStyles.boldLabel);
            ...
            r = EditorGUILayout.GetControlRect();
            EditorGUI.LabelField(r, "旋转噪声 - 振幅单位为度", EditorStyles.boldLabel);
            r = EditorGUILayout.GetControlRect(true, mPreviewHeight * EditorGUIUtility.singleLineHeight);
            if (Event.current.type == EventType.Repaint)
            {
            ...
            SerializedProperty property, GUIContent[] titles)
        {
            ...
            for (int i = 0; i < 3; ++i)
                lists[i] = SetupReorderableList(property, i, new GUIContent("组件"));
            return lists;
        }
            ...
            SerializedProperty property, int channel, GUIContent title)
        {
            ...
            list.drawHeaderCallback = (Rect rect) =>
                {
                    GUIContent steadyLabel = new GUIContent("(勾选后为非随机波形)");

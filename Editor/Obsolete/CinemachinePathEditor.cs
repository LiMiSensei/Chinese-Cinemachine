...
namespace Unity.Cinemachine.Editor
{
    ...
    class CinemachinePathEditor : BaseEditor<CinemachinePath>
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            GUILayout.Label(new GUIContent("选中的路径点："));
            ...
            else
            {
                if (Target.m_Waypoints.Length > 0)
                {
                    EditorGUI.HelpBox(rect,
                        "在场景视图或 “路径详情” 列表中点击一个路径点",
                        MessageType.Info);
                }
                else if (GUI.Button(rect, new GUIContent("向路径添加一个路径点")))
                {
            ...
            if (mPreferTangentSelection != EditorGUILayout.Toggle(
                    new GUIContent("优先拖动切线",
                        "编辑路径时，若路径点位置与切线重合，拖动操作将优先作用于切线"),
        ...
        void DrawWaypointEditor(Rect rect, int index)
        {
            ...
            if (GUI.Button(r, new GUIContent(index.ToString(), "前往场景视图中的路径点")))
            {

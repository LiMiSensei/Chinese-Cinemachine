        ...
        public override void OnInspectorGUI()
        {
            ...
            if (m_IsPrefabOrInPrefabMode)
                EditorGUILayout.HelpBox(
                    "仅可指定预制件内部的 Cinemachine 相机，且该属性必须暴露。", 
                    MessageType.Info);
            ...
            {
                GUIContent createLabel = new GUIContent("创建");
            ...
            EditorGUILayout.Space();
            EditorGUILayout.HelpBox(
                "为获得最佳检视面板（inspector）显示效果，请将时间线（Timeline）升级至 1.8.2 版本或更高版本。",
                MessageType.Info);
